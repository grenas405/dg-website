...
### Added
- `deploy/setup-vps.sh`: idempotent one-shot VPS setup automating the README runbook — installs and (re)starts the systemd service, waits for `/healthz` on loopback, issues the Let's Encrypt certificate on first run (standalone, then switches renewals to the zero-downtime webroot method and dry-runs a renewal), installs the nginx snippet + site config with `nginx -t` validation and reload, and finishes with a public HTTPS health check (non-fatal if DNS is not pointed yet).
