...
### Security
- TLS enabled at the Nginx edge: the shipped `deploy/nginx/denogenesis.com.conf` now terminates HTTPS (TLS 1.2/1.3, Mozilla "intermediate" ciphers, OCSP stapling, HTTP/2), 301-redirects all HTTP to HTTPS, and keeps an HTTP `/.well-known/acme-challenge/` carve-out for zero-downtime certbot renewals. The app's conditional HSTS now activates automatically. README documents the certbot bootstrap (DNS prerequisites, first issuance, webroot renewal).
